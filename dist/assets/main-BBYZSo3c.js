import"./beacon-sync-DBXmqi-W.js";window.addEventListener("error",M=>{console.error("Global JS Error Captured in app.js:",M.message,"at",M.filename,":",M.lineno)});window.showToast=M=>{const Y=document.getElementById("playground-toast");Y?(Y.innerText=M,Y.classList.add("active"),setTimeout(()=>{Y.classList.remove("active")},2200)):console.log("Toast:",M)};let A=[];const vt=M=>M.title.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,""),eo=M=>{const Y=vt(M),ae=`${window.location.origin}/?package=${Y}`,S=M.priceStr||M.price,N=M.basePrice||Math.round(M.priceNum*1.11),H=Math.round((N-M.priceNum)/N*100),J="₹"+N.toLocaleString(),Te=(M.highlights?M.highlights.split(","):["Luxury Stays","Guided Activities","Transfers Included"]).slice(0,4).map(K=>K.trim()).join(" • ");let $e="";return H>0&&($e=`
${H}% OFF • Original ${J}`),`*${M.title}*

${M.duration} Days / ${M.duration-1} Nights
${M.location?M.location.split(",")[0]:M.category.toUpperCase()}
...
${ae}`},to=(M,Y)=>{const ae=document.getElementById("share-canvas");if(!ae)return;const S=ae.getContext("2d"),N=600,H=800;ae.width=N,ae.height=H;const J=new Image;J.crossOrigin="anonymous",J.onload=()=>{const he=J.width/J.height,Te=N/H;let $e,K,de,ne;he>Te?(ne=J.height,de=J.height*Te,$e=(J.width-de)/2,K=0):(de=J.width,ne=J.width/Te,$e=0,K=(J.height-ne)/2),S.drawImage(J,$e,K,de,ne,0,0,N,H);const st=S.createLinearGradient(0,0,0,100);st.addColorStop(0,"rgba(7, 7, 28, 0.7)"),st.addColorStop(1,"rgba(7, 7, 28, 0)"),S.fillStyle=st,S.fillRect(0,0,N,100);const Ge=S.createLinearGradient(0,H*.55,0,H);Ge.addColorStop(0,"rgba(7, 7, 28, 0)"),Ge.addColorStop(.3,"rgba(7, 7, 28, 0.5)"),Ge.addColorStop(.7,"rgba(7, 7, 28, 0.9)"),Ge.addColorStop(1,"rgba(7, 7, 28, 0.98)"),S.fillStyle=Ge,S.fillRect(0,H*.55,N,H*.45),S.strokeStyle="#00CBE0",S.lineWidth=4,S.strokeRect(2,2,N-4,H-4),S.strokeStyle="#00CBE0",S.lineWidth=3.5,S.beginPath(),S.arc(45,45,14,.2*Math.PI,1.6*Math.PI),S.stroke(),S.strokeStyle="#00CBE0",S.lineWidth=2.5,S.beginPath(),S.moveTo(56,32),S.lineTo(62,38),S.moveTo(62,32),S.lineTo(56,38),S.stroke(),S.fillStyle="#ffffff",S.font='900 16px "Poppins", sans-serif',S.fillText("BEACON",75,47),S.fillStyle="#00CBE0",S.font='700 10px "Poppins", sans-serif',S.fillText("TRAVEL",75,58);const Mt=M.rating.replace("⭐","").trim();S.fillStyle="rgba(7, 7, 28, 0.75)",S.strokeStyle="rgba(255, 255, 255, 0.1)",S.lineWidth=1,S.beginPath(),S.roundRect(N-110,25,85,38,6),S.fill(),S.stroke(),S.fillStyle="#FFD700",S.font='14px "Poppins", sans-serif',S.fillText("⭐",N-100,48),S.fillStyle="#ffffff",S.font='800 13px "Poppins", sans-serif',S.fillText(Mt,N-78,48);const Dt=(M.style||M.category||"FEATURED").toUpperCase();S.fillStyle="#00CBE0",S.font='800 11px "Poppins", sans-serif',S.fillText(Dt,45,H-145);const _e=(M.location?M.location.split(",")[0]:M.category||"JOURNEY").toUpperCase();S.fillStyle="#ffffff",S.font='900 48px "Poppins", sans-serif',S.fillText(_e,45,H-90),S.fillStyle="rgba(255, 255, 255, 0.85)",S.font='500 20px "Poppins", sans-serif',S.fillText(M.title,45,H-55),S.fillStyle="#00CBE0",S.font='800 10px "Poppins", sans-serif',S.fillText("EXPLORE THIS JOURNEY ON BEACON",45,H-25);const wt=ae.toDataURL("image/png");Y&&Y(wt)},J.onerror=()=>{S.fillStyle="#07071C",S.fillRect(0,0,N,H),S.strokeStyle="#00CBE0",S.lineWidth=4,S.strokeRect(2,2,N-4,H-4),S.fillStyle="#fff",S.font='900 28px "Poppins", sans-serif',S.textAlign="center",S.fillText("BEACON TRAVEL",N/2,H/2-40),S.fillStyle="#00CBE0",S.fillText(M.title.toUpperCase(),N/2,H/2+10);const he=ae.toDataURL("image/png");Y&&Y(he)},J.src=M.bg||M.imgUrl};window.openShareSheet=M=>{const Y=document.getElementById("beacon-share-sheet");if(!Y)return;const ae=document.getElementById("whatsapp-text-preview"),S=document.getElementById("share-image-preview"),N=eo(M);ae.innerText=N,document.body.classList.add("modal-open"),Y.style.display="flex",to(M,K=>{S.src=K}),Y.ontouchmove=K=>{K.target===Y&&K.preventDefault()};const H=document.getElementById("btn-close-share-sheet");H.onclick=()=>{document.body.classList.remove("modal-open"),Y.style.display="none"};const J=document.getElementById("btn-share-whatsapp");J.onclick=()=>{const K=document.getElementById("share-canvas");if(K)try{K.toBlob(de=>{if(de)navigator.clipboard.write([new ClipboardItem({"image/png":de})]).then(()=>{showToast("📋 Card image copied! Paste it in WhatsApp to attach."),setTimeout(()=>{const ne=encodeURIComponent(N);window.open("https://api.whatsapp.com/send?text="+ne,"_blank")},1500)}).catch(()=>{const ne=encodeURIComponent(N);window.open("https://api.whatsapp.com/send?text="+ne,"_blank")});else{const ne=encodeURIComponent(N);window.open("https://api.whatsapp.com/send?text="+ne,"_blank")}},"image/png")}catch{const ne=encodeURIComponent(N);window.open("https://api.whatsapp.com/send?text="+ne,"_blank")}else{const de=encodeURIComponent(N);window.open("https://api.whatsapp.com/send?text="+de,"_blank")}};const he=document.getElementById("btn-copy-share-link");he&&(he.onclick=()=>{const K=vt(M),de=`${window.location.origin}/?package=${K}`;navigator.clipboard.writeText(de).then(()=>{showToast("📋 Package URL copied to clipboard!")})});const Te=document.getElementById("btn-copy-share-text");Te.onclick=()=>{navigator.clipboard.writeText(N).then(()=>{showToast("📋 WhatsApp details text copied to clipboard!")})};const $e=document.getElementById("btn-download-share-card");$e.onclick=()=>{const K=document.getElementById("share-canvas");if(K){const de=K.toDataURL("image/png"),ne=document.createElement("a");ne.download=`${vt(M)}-share.png`,ne.href=de,ne.click(),showToast("💾 Downloaded Travel Card!")}}};const ao=()=>{const Y=new URLSearchParams(window.location.search).get("package"),ae=window.location.hash;let S=null;if(Y)S=A.find(N=>vt(N)===Y);else if(ae&&ae.startsWith("#package-")){const N=ae.replace("#package-","");S=A.find(H=>vt(H)===N||H.title.toLowerCase().includes(decodeURIComponent(N).toLowerCase()))}else{const N=window.location.pathname.split("/"),H=N.indexOf("package");if(H!==-1&&N[H+1]){const J=N[H+1];S=A.find(he=>vt(he)===J)}}S&&setTimeout(()=>{openPackageDetailsView(S)},300)};document.addEventListener("DOMContentLoaded",()=>{const M={agency:{name:"WanderGo Travels",legalName:"WanderGo Tour Operators Pvt Ltd",logo:"https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&w=150&q=80",address:"123 Example Road, Pune, Maharashtra - 411001",phone:"+91 99999 88888",email:"booking@wandergo.com",gstRegistered:!0,gstin:"27ABCDE1234F1Z5"},freelancer:{name:"ROHAN TRAVEL EXPERIENCES",legalName:"Rohan Mehta Experiences",logo:"https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80",address:"Pune, Maharashtra - 411038",phone:"+91 98765 43210",email:"rohan.experiences@planner.com",gstRegistered:!1,gstin:""}},Y=e=>{if(e.receiptSnapshot)return e.receiptSnapshot;const t=e.id==="BC-2026-9921"?M.agency:M.freelancer,a=e.priceNum||37e3,n=3e3,o=550,s=1e3,i=2e3,d=a+n+o+s-i,c=t.gstRegistered?.05:0,g=Math.round(d*c),u=d+g;e.receiptSnapshot={receiptNo:`BRC-2026-${Math.floor(1e5+Math.random()*9e5)}`,bookingId:e.id,paymentStatus:"PAID",paymentDate:"31 July 2026",bookingDate:"30 July 2026",utrId:"992100887321",verification:"Verified",planner:t,customer:{name:"Aditya Kasod",phone:"+91 98765 43210",email:"aditya.kasod@example.com"},trip:{packageTitle:e.packageTitle,destination:e.id==="BC-2026-9921"?"Kerala Backwaters":"Munnar Hills",dates:e.dateRange,duration:"5 Days / 4 Nights"},travellers:[{name:"Aditya Kasod",type:"Adult"},{name:"Riya Sharma",type:"Adult"},{name:"Aarav Sharma",type:"Child"},{name:"Myra Sharma",type:"Infant"}],customizations:{stay:"Sea View Resort Upgrade",transport:"Airport Transfer Sedan Cab — Included",meals:[{name:"Aditya Kasod",pref:"Vegetarian Throughout"},{name:"Riya Sharma",pref:"Non-Veg Throughout"},{name:"Aarav Sharma",pref:"Vegetarian Throughout"}],addons:["Beach Candlelight Dinner","Airport lounge Lounge access"]},pricing:{basePrice:a,upgradesPrice:n,mealsPrice:o,addonsPrice:s,discount:i,subtotal:d,taxes:g,grandTotal:u}};const r=JSON.parse(localStorage.getItem("beacon_bookings"))||[],h=r.findIndex(m=>m.id===e.id);return h!==-1&&(r[h]=e,localStorage.setItem("beacon_bookings",JSON.stringify(r))),e.receiptSnapshot},ae=()=>{document.querySelectorAll(".card-favorite-btn").forEach(e=>{e.querySelector("svg")||(e.innerHTML='<svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>')}),document.querySelectorAll(".card-share-btn").forEach(e=>{e.querySelector("svg")||(e.innerHTML='<svg viewBox="0 0 24 24"><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon><line x1="22" y1="2" x2="11" y2="13"></line></svg>')})};ae();const S=()=>{document.querySelectorAll(".dest-tab.active, .exp-tab.active, .style-tab.active, .budget-tab.active, .seasonal-tab.active").forEach(t=>{t.click()}),ae()},N=document.getElementById("liquid-glass-navigation");N&&window.addEventListener("scroll",()=>{window.scrollY>80?N.classList.add("scrolled"):N.classList.remove("scrolled")});const H=document.querySelectorAll(".hero5-dot"),J=document.getElementById("hero5-title"),he=document.getElementById("hero5-tag"),Te=document.getElementById("hero5-desc"),$e=[{title:"Life Beyond Routine",tag:"Nature's Whispers",desc:"Handpicked journeys. Unforgettable experiences. Made for you.",image:"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1920&q=80"},{title:"Discover Golden Sands",tag:"Coastal Paradises",desc:"Immerse yourself in crystal waters and scenic shorelines curated by travel vloggers.",image:"https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?auto=format&fit=crop&w=1920&q=80"},{title:"Roam Hidden Valleys",tag:"Let's Explore The World",desc:"Zen gardens, mountain trails, and isolated lodges crafted for wellness seekers.",image:"https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?auto=format&fit=crop&w=1920&q=80"}];let K=0,de;function ne(e){H.forEach(a=>a.classList.remove("active")),H[e]&&H[e].classList.add("active");const t=$e[e];if(t){J&&(J.style.opacity="0"),he&&(he.style.opacity="0"),Te&&(Te.style.opacity="0");const a=document.getElementById("hero5-banner");a&&(a.style.backgroundImage=`url('${t.image}')`),setTimeout(()=>{J&&(J.innerText=t.title,J.style.opacity="1"),he&&(he.innerText=t.tag,he.style.opacity="1"),Te&&(Te.innerText=t.desc,Te.style.opacity="1")},300)}}function st(){de=setInterval(()=>{K=(K+1)%$e.length,ne(K)},1e4)}function Ge(){clearInterval(de),st()}H.forEach((e,t)=>{e.addEventListener("click",()=>{K=t,ne(t),Ge()})}),st();const Mt=e=>{let t=document.getElementById("share-toast");t||(t=document.createElement("div"),t.id="share-toast",t.style.cssText=`
                position: fixed;
                bottom: 30px;
                right: 30px;
                background: #0ea5e9;
...
`)}),e.innerHTML=n;const s=`
        ...
        `,a.querySelector("#btn-wait-close").onclick=()=>{a.style.display="none",clearInterval(gt)},gt=setInterval(()=>{const s=(JSON.parse(localStorage.getItem("beacon_payment_attempts"))||[]).find(i=>i.bookingId===e);s&&s.status==="PLANNER_CONFIRMED"?(clearInterval(gt),n.innerHTML=`
                ...
                `,a.querySelector("#btn-wait-view-booking").onclick=()=>{a.style.display="none",Ze(),_("bookings",!0)}):s&&s.status==="PAYMENT_REVIEW_REQUIRED"&&(clearInterval(gt),n.innerHTML=`
                    <div style="font-size: 48px; margin-bottom: 20px;">❌</div>
        ...
        `},Vn=e=>{let t=JSON.parse(localStorage.getItem("beacon_recent_views"))||[];t=t.filter(a=>a.title!==e.title),t.unshift({title:e.title,viewedAt:Date.now()}),t=t.slice(0,15),localStorage.setItem("beacon_recent_views",JSON.stringify(t))},jn=e=>{const t=document.getElementById("similar-packages-grid");if(!t)return;let a=A.filter(i=>i.title!==e.title);a.forEach(i=>{let d=0;i.category===e.category&&(d+=40),i.style===e.style&&(d+=25);const c=Math.abs(i.priceNum-e.priceNum)/e.priceNum;c<=.3&&(d+=Math.round((1-c)*25)),Math.abs(i.duration-e.duration)<=1&&(d+=10),i.similarityScore=d}),a.sort((i,d)=>d.similarityScore-i.similarityScore);const n=a.slice(0,4);let o="";n.forEach(i=>{const d=Math.min(96,Math.max(78,Math.round(75+i.similarityScore/1.5))),c=X.includes(i.title),g=j.some(r=>r.title===i.title);let u="Popular choice";i.category===e.category?u="Same Destination":i.style===e.style?u=`Similar ${i.style}`:Math.abs(i.priceNum-e.priceNum)/e.priceNum<=.15&&(u="Similar budget"),o+=`
            ...
            `}),t.innerHTML=o,t.querySelectorAll(".rec-travel-card").forEach((i,d)=>{const c=n[d];i.addEventListener("click",r=>{r.target.closest("button")||Se(c)});const g=i.querySelector(".btn-rec-save");g.addEventListener("click",r=>{r.stopPropagation(),We(c.title);const h=X.includes(c.title);g.innerHTML=h?"❤️":"♡",g.style.color=h?"var(--accent-cyan)":"var(--text-slate)";const m=document.querySelector(".details-save-btn");m&&c.title===e.title&&(m.innerHTML=h?"❤️ Saved":"♡ Save")});const u=i.querySelector(".btn-rec-compare");u.addEventListener("click",r=>{r.stopPropagation();const h=j.some(v=>v.title===c.title);dt(c,!h);const m=j.some(v=>v.title===c.title);u.innerHTML=m?"✓":"+",u.style.color=m?"var(--accent-cyan)":"var(--text-slate)"})})},nn=(e,t)=>{const a=document.getElementById("view-planner-profile");if(!a)return;_("planner-profile",!0);const n=e.name==="Rahul Mehta",o=A.filter(c=>c.title===t.title?!1:n?c.category==="beaches"||c.style==="couple":c.category==="mountains"||c.category==="nature"||c.category==="spiritual").slice(0,4);let s="";o.forEach(c=>{s+=`
        ...
        `;const i=a.querySelector(".btn-profile-back");i&&i.addEventListener("click",()=>{if(_("package-details",!0),window.innerWidth<768){ht(pkg);return}}),a.querySelectorAll(".travel-card").forEach((c,g)=>{c.addEventListener("click",()=>{const u=o[g];u&&Se(u)})})},Se=e=>{kt=e,Vn(e);let t="My Collection";Je==="home"?t="Home":Je==="planner"?t="Trip Planner":Je==="collection"&&(t="My Collection");const a=X.includes(e.title),n=document.getElementById("match-start-date")?document.getElementById("match-start-date").value:"",o=document.getElementById("match-end-date")?document.getElementById("match-end-date").value:"";if(n&&o){const l=f=>{const x=f.split("-");if(x.length===3){const P=new Date(x[0],x[1]-1,x[2]),ie=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return`${P.getDate()} ${ie[P.getMonth()]} ${P.getFullYear()}`}return f};`${l(n)}${l(o)}`}else{const l=new Date;l.toLocaleString("default",{month:"short"}),new Date(l.setMonth(l.getMonth()+3)).toLocaleString("default",{month:"short"}),new Date().getFullYear()}const s=j.some(l=>l.title===e.title),i=document.getElementById("view-package-details");if(!i)return;if(_("package-details",!0),window.innerWidth<768){ht(e);return}""+(e.basePrice||Math.round(e.priceNum*1.11)).toLocaleString();const c=e.inclusions?e.inclusions.split(",").map(l=>`<li>✓ ${l.trim()}</li>`).join(""):`
                ...
                <div class="itinerary-day-card" data-day="${l}">
                    ...
                    <div class="itinerary-day-body">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; background: rgba(255,255,255,0.01); border: 1px solid rgba(255,255,255,0.03); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                            <div style="font-size: 12.5px; color: var(--text-slate);">🚕 <strong>Transport:</strong><br>${P}</div>
        ...
        `,window.scrollTo({top:0,behavior:"instant"});const b=i.querySelector(".btn-details-back");b&&b.addEventListener("click",()=>{_(Je)});const I=i.querySelectorAll(".itinerary-day-card");I.forEach(l=>{l.querySelector(".itinerary-day-header").addEventListener("click",()=>{l.classList.contains("active")?l.classList.remove("active"):l.classList.add("active")})});const E=i.querySelector(".btn-toggle-all-days");E&&E.addEventListener("click",()=>{const l=Array.from(I).some(f=>!f.classList.contains("active"));I.forEach(f=>{l?f.classList.add("active"):f.classList.remove("active")}),E.innerText=l?"Collapse All Days":"View All Days"});const C=i.querySelectorAll(".policy-accordion-card");C.forEach(l=>{l.querySelector(".policy-accordion-header").addEventListener("click",()=>{const x=l.classList.contains("active");C.forEach(P=>P.classList.remove("active")),x||l.classList.add("active")})});const T=i.querySelectorAll(".gallery-thumb"),z=i.querySelector(".gallery-primary-img");T.forEach(l=>{l.addEventListener("click",()=>{T.forEach(f=>f.classList.remove("active")),l.classList.add("active"),z.style.backgroundImage=l.style.backgroundImage})});const V=i.querySelectorAll(".details-nav-item");V.forEach(l=>{l.addEventListener("click",()=>{V.forEach(x=>x.classList.remove("active")),l.classList.add("active");const f=i.querySelector(`#sec-${l.dataset.sec}`);f&&f.scrollIntoView({behavior:"smooth",block:"start"})})});const le=i.querySelectorAll(".details-section-block"),F=()=>{if(ve!=="package-details")return;const l=window.scrollY+150;le.forEach(f=>{if(l>=f.offsetTop&&l<f.offsetTop+f.offsetHeight){const x=f.id.replace("sec-","");V.forEach(P=>{P.dataset.sec===x?P.classList.add("active"):P.classList.remove("active")})}})};window.addEventListener("scroll",F);const q=i.querySelectorAll(".details-save-btn"),G=()=>{q.forEach(l=>{X.includes(e.title)?(l.innerHTML="❤️ Saved",l.style.color="var(--accent-cyan)",l.style.borderColor="var(--accent-cyan)"):(l.innerHTML="♡ Save",l.style.color="#fff",l.style.borderColor="rgba(255, 255, 255, 0.08)")})};G(),q.forEach(l=>{l.addEventListener("click",()=>{We(e.title),G()})});const U=i.querySelectorAll(".details-compare-btn"),Z=()=>{U.forEach(l=>{j.some(f=>f.title===e.title)?(l.innerHTML="✓ Compared",l.style.color="var(--accent-cyan)",l.style.borderColor="var(--accent-cyan)"):(l.innerHTML="+ Compare",l.style.color="#fff",l.style.borderColor="rgba(255, 255, 255, 0.08)")})};Z(),U.forEach(l=>{l.addEventListener("click",()=>{const f=!j.some(x=>x.title===e.title);dt(e,f),Z()})});const oe=i.querySelector(".details-share-btn");oe&&oe.addEventListener("click",()=>{openShareSheet(e)}),i.querySelectorAll(".details-enquiry-btn").forEach(l=>{l.addEventListener("click",()=>{showToast("📩 Redirecting to Enquiries page..."),_("enquiries")})}),i.querySelectorAll(".details-book-btn").forEach(l=>{l.addEventListener("click",()=>{ka(e)})});const ge=i.querySelector(".btn-view-full-profile");ge&&ge.addEventListener("click",()=>{nn(r,e)}),jn(e)},ot=()=>{const e=document.getElementById("collection-saved-list");if(e){if(X.length===0){e.innerHTML=`
            ...
            `;return}e.innerHTML="",X.forEach(t=>{const a=A.find(i=>i.title.trim()===t.trim())||{title:t,imgUrl:"https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=400&q=80",priceStr:"₹14,999",priceNum:14999,category:"beaches",duration:4,rating:"⭐ 4.8",style:"family"},n=j.some(i=>i.title===a.title),o=document.createElement("div");o.className="saved-card-horizontal",o.innerHTML=`
            ...
            `,o.style.cursor="pointer",o.addEventListener("click",i=>{i.target.closest(".btn-add-compare")||i.target.closest(".btn-remove")||Se(a)});const s=o.querySelector(".btn-add-compare");s.addEventListener("click",()=>{const i=!j.some(c=>c.title===a.title);dt(a,i),s.innerText=i?"Added to Compare":"Add to Compare";const d=document.querySelector(`.card-compare-checkbox[data-title="${a.title}"]`);d&&(d.checked=i)}),o.querySelector(".btn-remove").addEventListener("click",()=>{o.style.maxHeight=o.offsetHeight+"px",o.style.transition="all 0.3s",o.style.opacity="0",o.style.transform="scale(0.95)",setTimeout(()=>{We(a.title)},300)}),e.appendChild(o)})}},bt=()=>{const e=document.getElementById("compare-selection-list"),t=document.getElementById("compare-trigger-row"),a=document.getElementById("comparison-results-view");if(e){if(j.length===0){e.innerHTML=`
            ...
            `,t.style.display="none",a.style.display="none";return}e.innerHTML="",t.style.display="block",j.forEach(n=>{const o=document.createElement("div");o.className="compare-selection-card selected",o.style.cursor="pointer",o.innerHTML=`
                <span class="card-title">${n.title}</span>
                <span class="compare-checkbox-indicator">✓</span>
            `,o.addEventListener("click",()=>{dt(n,!1);const s=document.querySelector(`.card-compare-checkbox[data-title="${n.title}"]`);s&&(s.checked=!1),bt()}),e.appendChild(o)}),j.length>=2?on():a.style.display="none"}},on=()=>{const e=document.getElementById("comparison-results-view"),t=document.getElementById("collection-compare-table"),a=document.getElementById("ai-compare-insight-card");if(!t||!e)return;e.style.display="block";let n=0,o=0,s=0,i=-1,d=1/0,c=-1;j.forEach((r,h)=>{const m=parseFloat(r.rating.replace(/[^0-9.]/g,""))||4.5;r.matchScore=Math.round(90+m*2-r.priceNum/15e4*5),r.matchScore>i&&(i=r.matchScore,n=h),r.priceNum<d&&(d=r.priceNum,o=h),m>c&&(c=m,s=h)}),a&&(a.innerHTML=Nn(j));let g='<thead><tr><th class="matrix-header-col">Attributes</th>';j.forEach((r,h)=>{let m="";h===n&&(m+='<span class="compare-badge-ribbon ribbon-best-match">Best Match</span><br>'),h===o&&(m+='<span class="compare-badge-ribbon ribbon-best-price">Best Price</span><br>'),h===s&&(m+='<span class="compare-badge-ribbon ribbon-best-rated">Best Rated</span><br>'),g+=`<th style="vertical-align: bottom;">
                <div style="min-height: 50px;">${m}</div>
                <div class="matrix-pkg-title" style="margin-top: 5px;">${r.title}</div>
            </th>`}),g+="</tr></thead><tbody>",[{label:"Destination",key:"category",highlight:!1,format:r=>r.toUpperCase()},{label:"Price / Person",key:"priceStr",highlight:!0},{label:"Duration",key:"duration",highlight:!1,format:r=>`${r} Days`},{label:"Rating",key:"rating",highlight:!0},{label:"Hotel",key:"accommodation",highlight:!1,format:r=>r?r.charAt(0).toUpperCase()+r.slice(1):"4★ Accommodation"},{label:"Meals",key:"meals",highlight:!1,format:r=>r||"Breakfast Included"},{label:"Transportation",key:"transport",highlight:!1,format:r=>r?r.charAt(0).toUpperCase()+r.slice(1):"Private Cab"},{label:"Activities",key:"highlights",highlight:!1,format:r=>`${r?r.split(",").length:3} Major Activities`},{label:"Flights",key:"transport",highlight:!1,format:r=>r==="flight"?"✓ Included":"✕ Excluded"},{label:"Guide",key:"highlights",highlight:!1,format:r=>r&&(r.toLowerCase().includes("guide")||r.toLowerCase().includes("naturalist"))?"✓ Certified Guide":"✓ Local Assist"},{label:"Trip Type",key:"style",highlight:!1,format:r=>r?r.charAt(0).toUpperCase()+r.slice(1):"Family"},{label:"Cancellation Policy",key:"cancellation",highlight:!1,format:r=>r||"Free cancellation within 24 hours"},{label:"Planner / Company",key:"planner",highlight:!1,format:r=>r||"Wanderlust Travels"},{label:"Beacon Match %",key:"matchScore",highlight:!0,format:r=>`${r}% Match`}].forEach(r=>{g+=`<tr><td class="matrix-header-col">${r.label}</td>`,j.forEach((h,m)=>{const v=h[r.key],y=r.format?r.format(v):v||"N/A";g+=`<td class="${r.highlight?"highlight-diff":""}">${y}</td>`}),g+="</tr>"}),g+='<tr><td class="matrix-header-col" style="border-bottom: none;">Actions</td>',j.forEach(r=>{g+=`<td style="border-bottom: none;">
                <div style="display: flex; gap: 10px;">
            ...
            </td>`}),g+="</tr></tbody>",t.innerHTML=g,t.querySelectorAll(".view-pkg-compare").forEach(r=>{r.addEventListener("click",()=>{const h=r.dataset.title,m=A.find(v=>v.title===h);m&&Se(m)})}),t.querySelectorAll(".send-enquiry-compare").forEach(r=>{r.addEventListener("click",()=>{showToast("📩 Redirecting to Enquiries page..."),_("enquiries")})})},sn=()=>{const e=document.getElementById("collection-recent-list");if(!e)return;const t=JSON.parse(localStorage.getItem("beacon_recent_views"))||[];if(t.length===0){e.innerHTML=`
                <div class="collection-empty-state">
                    <span class="empty-icon">⏳</span>
            ...
            `;return}e.innerHTML="";const a={Today:[],Yesterday:[],Earlier:[]};t.forEach(n=>{const o=A.find(s=>s.title.trim()===n.title.trim());if(o){const s=qn(n.viewedAt);a[s].push({pkg:o,timestamp:n.viewedAt})}});for(let n in a){const o=a[n];if(o.length===0)continue;const s=document.createElement("div");s.className="timeline-group-header",s.innerText=n,e.appendChild(s);const i=document.createElement("div");i.className="recent-timeline-cards",o.forEach(d=>{const c=d.pkg,g=X.includes(c.title),u=document.getElementById("match-start-date")?document.getElementById("match-start-date").value:"",r=document.getElementById("match-end-date")?document.getElementById("match-end-date").value:"";if(u&&r){const b=I=>{const E=I.split("-");if(E.length===3){const C=new Date(E[0],E[1]-1,E[2]),T=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return`${C.getDate()} ${T[C.getMonth()]} ${C.getFullYear()}`}return I};`${b(u)}${b(r)}`}else{const b=new Date;b.toLocaleString("default",{month:"short"}),new Date(b.setMonth(b.getMonth()+3)).toLocaleString("default",{month:"short"}),new Date().getFullYear()}const h=j.some(b=>b.title===c.title),m=document.createElement("div");m.className="saved-card-horizontal",m.innerHTML=`
                    <div class="card-img-col" style="background-image: url('${c.imgUrl}')"></div>
                    <div class="card-details-col">
                        <div>
                            <div class="card-title-row">
                                <h4 class="card-title">${c.title}</h4>
                                <span style="font-size: 11px; font-weight: 700; color: var(--accent-cyan);">${Rn(d.timestamp)}</span>
                            </div>
                            <div class="card-meta">
            ...
            `},o=y=>{y.querySelectorAll(".mobile-compact-card").forEach(b=>{const I=b.dataset.title,E=A.find(q=>q.title===I);if(!E)return;b.onclick=q=>{q.target.closest(".mobile-card-fav-btn")||q.target.closest(".mobile-card-share-btn")||q.target.closest(".mobile-card-like-btn")||Se(E)};const C=b.querySelector(".mobile-card-like-btn");C&&(C.onclick=q=>{q.stopPropagation();const U=!(localStorage.getItem("liked_"+E.title)==="true");localStorage.setItem("liked_"+E.title,U?"true":"false"),U?C.classList.add("active"):C.classList.remove("active"),C.classList.add("pop-bounce"),setTimeout(()=>C.classList.remove("pop-bounce"),300),window.showToast(U?"Liked package!":"Unliked package")});const T=b.querySelector(".mobile-card-share-btn");T&&(T.onclick=q=>{q.stopPropagation(),openShareSheet(E)});const z=b.querySelector(".mobile-card-fav-btn");z&&(z.onclick=q=>{q.stopPropagation(),toggleSavedPackage(E.title);const G=X.includes(E.title);G?z.classList.add("active"):z.classList.remove("active"),z.classList.add("pop-bounce"),setTimeout(()=>z.classList.remove("pop-bounce"),300);const U=document.getElementById("playground-toast");U&&(U.innerText=G?"Saved to My Collection":"Removed from Collection",U.classList.add("active"),setTimeout(()=>U.classList.remove("active"),2200)),pt(),Ue()});let V=null;const le=q=>{V=setTimeout(()=>{const G=document.getElementById("playground-preview-sheet"),U=document.getElementById("playground-preview-backdrop"),Z=document.getElementById("preview-sheet-title"),oe=document.querySelector("#playground-preview-sheet p"),Me=document.querySelector('#playground-preview-sheet span[style*="accent-cyan"]'),De=document.querySelector('#playground-preview-sheet span[style*="font-size: 12px"]'),ge=document.getElementById("btn-close-sheet-preview");G&&U&&(Z&&(Z.innerText=E.title),oe&&(oe.innerText=E.highlights||E.description||"Experience the best of local food, sightseeing, and scenic guides customized for your comfort."),Me&&(Me.innerText=E.price||E.priceStr),De&&(De.innerText=`⏱ ${E.duration} • ${E.type||E.category.toUpperCase()}`),ge&&(ge.onclick=()=>{G.classList.remove("active"),setTimeout(()=>U.style.display="none",300),Se(E)}),U.style.display="block",setTimeout(()=>G.classList.add("active"),50))},500)},F=()=>{V&&clearTimeout(V)};b.addEventListener("mousedown",le),b.addEventListener("touchstart",le),b.addEventListener("mouseup",F),b.addEventListener("touchend",F),b.addEventListener("mouseleave",F)})},s=e.querySelectorAll(".mobile-chip");s.forEach(y=>{y.onclick=()=>{s.forEach(I=>I.classList.remove("active")),y.classList.add("active");const b=y.dataset.filter;i(b)}});const i=y=>{["rail-top-picks","rail-trending","rail-weekend","rail-adventure","rail-beaches","rail-more-journeys"].forEach(I=>{const E=document.getElementById(I);if(!E)return;const C=E.querySelector(".rail-items-scroll");let T=[];I==="rail-top-picks"?T=A.slice(0,3):I==="rail-trending"?T=[...A].reverse().slice(0,4):I==="rail-weekend"?T=A.filter(z=>parseInt(d(z))<=4):I==="rail-adventure"?T=A.filter(z=>z.style==="adventure"||z.category==="mountains"||z.experiences==="trekking"||z.title&&z.title.toLowerCase().includes("expedition")):I==="rail-beaches"?T=A.filter(z=>z.category==="beaches"||z.experiences==="beaches"||z.title&&z.title.toLowerCase().includes("beach")):T=A.slice(2,6),y!=="all"&&(T=T.filter(z=>z.category===y)),T.length===0?E.style.display="none":(E.style.display="block",C.innerHTML=T.map(z=>n(z)).join(""),o(C))})},d=y=>y.duration===void 0||y.duration===null?"5":String(y.duration).replace(/[^0-9]/g,"")||"5",c=()=>{const y=document.getElementById("mobile-featured-carousel");if(!y)return;const b=[A.find(T=>T.title.includes("Ladakh"))||A[0],A.find(T=>T.title.includes("Goa"))||A[1],A.find(T=>T.title.includes("Kashmir"))||A[2]].filter(Boolean);let I=0;const E=()=>{const T=b[I],z=X.includes(T.title),V=document.getElementById("match-start-date")?document.getElementById("match-start-date").value:"",le=document.getElementById("match-end-date")?document.getElementById("match-end-date").value:"";if(V&&le){const q=G=>{const U=G.split("-");if(U.length===3){const Z=new Date(U[0],U[1]-1,U[2]),oe=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return`${Z.getDate()} ${oe[Z.getMonth()]} ${Z.getFullYear()}`}return G};`${q(V)}${q(le)}`}else{const q=new Date;q.toLocaleString("default",{month:"short"}),new Date(q.setMonth(q.getMonth()+3)).toLocaleString("default",{month:"short"}),new Date().getFullYear()}y.innerHTML=`
                    <div class="hero-carousel-item" style="background-image: url('${T.bg||T.imgUrl}')">
                        ...
                        <div class="hero-carousel-content">
                            ...
                            <div class="hero-carousel-buttons">
                                ...
                                <button type="button" class="btn-secondary-action btn-hero-save" style="flex: 1; padding: 10px; border-radius: 8px; font-weight: 800; font-size: 13px; display: flex; align-items: center; justify-content: center; gap: 6px;">${z?"❤️ Saved":"♡ Save"}</button>
                            </div>
                            
                            <!-- Pagination indicators -->
                            <div style="display: flex; gap: 6px; justify-content: center; margin-top: 15px;">
                                ${b.map((q,G)=>`<span class="carousel-dot" style="width: 6px; height: 6px; border-radius: 50%; background: ${G===I?"var(--accent-cyan)":"rgba(255,255,255,0.3)"};"></span>`).join("")}
                            </div>
                        </div>
                    </div>
                `,y.querySelector(".btn-hero-view").onclick=()=>{Se(T)};const F=y.querySelector(".btn-hero-save");F.onclick=()=>{toggleSavedPackage(T.title),E(),pt(),Ue()}};E();let C=0;y.ontouchstart=T=>{C=T.touches[0].clientX},y.ontouchend=T=>{const z=T.changedTouches[0].clientX-C;Math.abs(z)>50&&(z>0?I=(I-1+b.length)%b.length:I=(I+1)%b.length,E())}},g=()=>{const y=document.querySelector("#rail-experiences .rail-items-scroll");y&&(y.innerHTML=t.map(b=>`
            ...
            `).join(""),y.querySelectorAll(".destination-visual-card").forEach(b=>{b.onclick=()=>{const I=b.dataset.query,E=A.find(C=>C.title.toLowerCase().includes(I.toLowerCase()));E&&Se(E)}}))},r=()=>{const y=document.querySelectorAll(".budget-chip-item");y.forEach(b=>{b.onclick=()=>{y.forEach(T=>T.classList.remove("active")),b.classList.add("active");const I=parseInt(b.dataset.min),E=parseInt(b.dataset.max),C=A.find(T=>{const z=parseInt(T.price.replace(/[^0-9]/g,""))||15e3;return z>=I&&z<=E});C&&Se(C)}})},h=document.getElementById("btn-mobile-trigger-planner");h&&(h.onclick=()=>{_("planner")});const m=()=>{const y=document.getElementById("rail-continue-exploring");if(!y)return;const b=y.querySelector(".rail-items-scroll"),I=JSON.parse(localStorage.getItem("beacon_recent_views"))||[];if(I.length===0){y.style.display="none";return}y.style.display="block";const E=I.map(C=>A.find(T=>T.title===C.title)).filter(Boolean);b.innerHTML=E.map(C=>n(C)).join(""),o(b)},v=()=>{const y=JSON.parse(localStorage.getItem("beacon_recent_views"))||[],b=X[0];let I="",E="";if(b){const F=A.find(q=>q.title===b);F&&(I=F.category,E=F.title)}else if(y.length>0){const F=A.find(q=>q.title===y[0].title);F&&(I=F.category,E=F.title)}const C=document.getElementById("rail-because-liked");if(C&&C.remove(),!I)return;const T=document.createElement("div");T.className="mobile-rail-section",T.id="rail-because-liked",T.innerHTML=`
            ...
            `;const z=T.querySelector(".rail-items-scroll"),V=A.filter(F=>F.category===I&&F.title!==E);if(V.length===0)return;z.innerHTML=V.map(F=>n(F)).join(""),o(z);const le=document.getElementById("rail-more-journeys");le&&le.parentElement.insertBefore(T,le)};c(),i("all"),g(),u(),r(),m(),v()};setTimeout(()=>{ao(),S(),pt(),Ue();const e=document.getElementById("btn-open-animation-playground");e&&(e.onclick=()=>{_("playground")});const t=document.getElementById("btn-playground-back");t&&(t.onclick=()=>{_("profile")});const a=[{title:"Leh Ladakh Expedition",bg:"https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=400&q=80"},{title:"Goa Beachfront Escape",bg:"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=400&q=80"},{title:"Kashmir Paradise Escape",bg:"https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=400&q=80"}];let n=0;const o=document.getElementById("playground-mini-hero"),s=document.getElementById("playground-hero-title");let i=setInterval(()=>{o&&(n=(n+1)%a.length,o.style.opacity=.5,setTimeout(()=>{o.style.backgroundImage=`url('${a[n].bg}')`,s.innerText=a[n].title,o.style.opacity=1},300))},6e3);o&&(o.ontouchstart=()=>{clearInterval(i)});const d=document.getElementById("playground-fav-btn"),c=document.getElementById("playground-toast");d&&(d.onclick=()=>{d.classList.toggle("active"),d.classList.add("pop-bounce"),setTimeout(()=>d.classList.remove("pop-bounce"),300);const E=d.classList.contains("active");d.innerText=E?"❤️":"♡",c&&(c.innerText=E?"Saved to My Collection":"Removed from Collection",c.classList.add("active"),setTimeout(()=>c.classList.remove("active"),2500))});const g=document.querySelectorAll(".playground-chip-btn"),u=document.getElementById("playground-results-container");g.forEach(E=>{E.onclick=()=>{g.forEach(T=>T.classList.remove("active")),E.classList.add("active");const C=E.dataset.filter;u&&(u.classList.add("fade-out"),setTimeout(()=>{C==="m"?u.innerHTML="🗻 Leh Ladakh Expedition &bull; Manali Cedar Chalet":u.innerHTML="🏝️ Goa Beachfront Escape &bull; Varkala Cliffside Yoga",u.classList.remove("fade-out")},200))}});const r=document.getElementById("btn-playground-longpress"),h=document.getElementById("playground-preview-backdrop"),m=document.getElementById("playground-preview-sheet"),v=document.getElementById("btn-close-sheet-preview");let y=null;if(r){const E=()=>{y=setTimeout(()=>{m&&h&&(h.style.display="block",setTimeout(()=>m.classList.add("active"),50))},500)},C=()=>{y&&clearTimeout(y)};r.addEventListener("mousedown",E),r.addEventListener("touchstart",E),r.addEventListener("mouseup",C),r.addEventListener("touchend",C)}v&&(v.onclick=()=>{m&&h&&(m.classList.remove("active"),setTimeout(()=>h.style.display="none",300))}),h&&(h.onclick=()=>{m&&(m.classList.remove("active"),setTimeout(()=>h.style.display="none",300))});const b=document.getElementById("btn-trigger-progressive-load"),I=document.getElementById("sharp-img-test");b&&I&&(b.onclick=()=>{I.classList.remove("loaded"),setTimeout(()=>{I.classList.add("loaded")},800)}),Fn()},500);const ht=e=>{kt=e;const t=document.getElementById("view-package-details");if(!t)return;const a=X.includes(e.title);j.some(w=>w.title===e.title),""+(e.basePrice||Math.round(e.priceNum*1.11)).toLocaleString();const o=e.duration||4,s=e.highlights?e.highlights.split(","):["Explore local viewpoints","Guided nature walk","Leisure tour of surrounding villages"],i=e.sightseeing?e.sightseeing.split(","):["Main square","Local markets","Scenic viewpoints"];let d="";for(let w=1;w<=o;w++){let k="",D="",R="",ee="",fe="Breakfast Included";if(w===1)k="Arrival & Resort Check-in",R="Private Airport Transfer",ee="Welcome dinner & sunset leisure",D="Arrive at the destination. Private airport transfer directly to your premium resort for check-in. Relax and enjoy a welcome cocktail with sunset beach views.",fe=e.meals==="All Inclusive"?"All Inclusive Meals":"Welcome Dinner Included";else if(w===o)k="Breakfast & Departure",R="Private Airport Transfer",ee="Morning beach walk & checkout",D="Enjoy a delicious morning buffet breakfast. Complete checkout formalities and board your private cab to the airport with cherished memories.",fe="Breakfast Included";else{const Oe=(w-2)%s.length,Re=(w-2)%i.length;k=s[Oe]?s[Oe].trim():"Local Sightseeing Tour",R="Private Cab local transfer",ee=`Explore ${i[Re]?i[Re].trim():"scenic highlights"}`,D=`Set out on an exciting sightseeing tour visiting ${i[Re]?i[Re].trim():"scenic highlights"} and popular local attractions.`,fe=e.meals==="All Inclusive"?"All Inclusive Meals":"Breakfast & Dinner Included"}d+=`
        ...
        `;const b=document.getElementById("btn-details-m-back");b&&(b.onclick=()=>{const w=document.querySelector(".mobile-bottom-nav");w&&window.innerWidth<768&&(w.style.display="flex"),_(Je)});const I=document.getElementById("btn-details-m-fav");I&&(I.onclick=()=>{toggleSavedPackage(e.title);const w=X.includes(e.title),k=I.querySelector(".heart-svg");w?(I.classList.add("active"),k&&(k.setAttribute("fill","var(--accent-cyan)"),k.setAttribute("stroke","var(--accent-cyan)")),showToast("❤️ Saved to My Collection")):(I.classList.remove("active"),k&&(k.setAttribute("fill","none"),k.setAttribute("stroke","#fff")),showToast("🤍 Removed from Collection")),I.style.transform="scale(1.2)",setTimeout(()=>I.style.transform="",200),pt(),Ue()});const E=document.getElementById("btn-details-m-share");E&&(E.onclick=()=>{openShareSheet(e)});const C=document.getElementById("btn-about-read-more"),T=document.getElementById("about-trip-text");C&&T&&C.addEventListener("click",()=>{T.style.display==="block"?(T.style.display="-webkit-box",C.innerHTML="Read More ↓"):(T.style.display="block",C.innerHTML="Read Less ↑")});const z=document.getElementById("mobile-details-accordion");z&&z.querySelectorAll(".itinerary-day-header").forEach(w=>{w.onclick=()=>{const k=w.parentElement,D=k.querySelector(".itinerary-day-body"),R=k.querySelector(".acc-arrow");k.classList.contains("active")?(k.classList.remove("active"),D.style.maxHeight="0",R&&(R.style.transform="")):(k.classList.add("active"),D.style.maxHeight="300px",R&&(R.style.transform="rotate(180deg)"))}});const V=document.getElementById("btn-m-view-full-itinerary");V&&z&&V.addEventListener("click",()=>{const w=z.querySelectorAll(".itinerary-day-card"),k=Array.from(w).some(D=>!D.classList.contains("active"));w.forEach(D=>{const R=D.querySelector(".itinerary-day-body"),ee=D.querySelector(".acc-arrow");k?(D.classList.add("active"),R.style.maxHeight="300px",ee&&(ee.style.transform="rotate(180deg)")):(D.classList.remove("active"),R.style.maxHeight="0",ee&&(ee.style.transform=""))}),k?V.innerHTML="Hide Full Itinerary ↑":V.innerHTML="View Full Itinerary >"}),document.querySelectorAll(".policy-accordion-card-m").forEach(w=>{const k=w.querySelector(".policy-header-m"),D=w.querySelector(".policy-body-m"),R=w.querySelector(".policy-arrow-m");k&&D&&k.addEventListener("click",()=>{D.style.display==="none"?(D.style.display="block",R&&(R.style.transform="rotate(90deg)"),R&&(R.style.color="var(--accent-cyan)")):(D.style.display="none",R&&(R.style.transform=""),R&&(R.style.color=""))})});const F=document.querySelectorAll(".gallery-item-m"),q=document.getElementById("btn-gallery-view-all"),G=document.getElementById("full-gallery-modal"),U=document.getElementById("close-gallery-btn"),Z=document.getElementById("gallery-swipe-wrap"),oe=document.getElementById("gallery-index-label"),Me=(w=0)=>{if(G&&(G.style.display="flex",Z)){const k=Z.clientWidth;Z.scrollTo({left:w*k,behavior:"instant"}),oe&&(oe.innerText=`${w+1} / 3`)}};F.forEach((w,k)=>{w.addEventListener("click",()=>Me(k))}),q&&q.addEventListener("click",()=>Me(0)),U&&(U.onclick=()=>{G&&(G.style.display="none")}),Z&&Z.addEventListener("scroll",()=>{const w=Z.clientWidth,k=Math.round(Z.scrollLeft/w);oe&&(oe.innerText=`${k+1} / 3`)});const De=document.getElementById("btn-view-map-accommodation");De&&De.addEventListener("click",()=>{const w=encodeURIComponent(e.hotelAddress||e.hotelName||"Maldives");window.open(`https://www.google.com/maps/search/?api=1&query=${w}`,"_blank")});const ge=document.getElementById("btn-m-view-partner-profile");ge&&ge.addEventListener("click",()=>{nn(g,e)}),document.querySelectorAll(".similar-card-m").forEach(w=>{w.addEventListener("click",D=>{if(D.target.closest(".sim-fav-btn"))return;const R=w.dataset.title,ee=A.find(fe=>fe.title===R);ee&&ht(ee)});const k=w.querySelector(".sim-fav-btn");k&&k.addEventListener("click",D=>{D.stopPropagation();const R=k.dataset.title;toggleSavedPackage(R);const ee=X.includes(R),fe=k.querySelector("svg");ee?(fe.setAttribute("fill","var(--accent-cyan)"),fe.setAttribute("stroke","var(--accent-cyan)"),showToast("❤️ Saved to My Collection")):(fe.setAttribute("fill","none"),fe.setAttribute("stroke","#fff"),showToast("🤍 Removed from Collection")),pt(),Ue()})});const f=document.getElementById("btn-m-book-now-sticky");f&&(f.onclick=()=>{ka(e)}),closeBookingDrawerBtn&&(closeBookingDrawerBtn.onclick=()=>{bookingDrawer&&(bookingDrawer.style.display="none")}),document.getElementsByName("m-book-date");const x=document.getElementById("m-book-date-1"),P=document.getElementById("m-book-date-2"),ie=document.getElementById("m-book-date-3");x&&(x.innerText=u);const ue=w=>{const k=new Date;k.setMonth(k.getMonth()+w);const D=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return`${k.getDate()} ${D[k.getMonth()]} – ${k.getDate()+5} ${D[k.getMonth()]} ${k.getFullYear()}`};P&&(P.innerText=ue(1)),ie&&(ie.innerText=ue(2));const it=document.getElementById("btn-m-travellers-dec"),qe=document.getElementById("btn-m-travellers-inc"),B=document.getElementById("m-travellers-count"),L=document.getElementById("lbl-m-travellers-breakdown");let $=2;const O=w=>{$=Math.max(1,Math.min(10,$+w)),B&&(B.innerText=$),L&&(L.innerText=$),Ie()};it&&(it.onclick=()=>O(-1)),qe&&(qe.onclick=()=>O(1));const ce=document.getElementById("m-addon-guide"),W=document.getElementById("m-addon-meals");ce&&(ce.onchange=()=>Ie()),W&&(W.onchange=()=>Ie());function Ie(){const k=(e.priceNum||22e3)*$;let D=0;ce&&ce.checked&&(D+=3500),W&&W.checked&&(D+=2e3*$);const R=Math.round((k+D)*.05),ee=k+D+R,fe=document.getElementById("breakdown-base-total"),Oe=document.getElementById("breakdown-addons-total"),Re=document.getElementById("breakdown-taxes-total"),At=document.getElementById("breakdown-grand-total");fe&&(fe.innerText="₹"+k.toLocaleString()),Oe&&(Oe.innerText="₹"+D.toLocaleString()),Re&&(Re.innerText="₹"+R.toLocaleString()),At&&(At.innerText="₹"+ee.toLocaleString())}const Le=document.getElementById("btn-m-confirm-booking-pay");Le&&(Le.onclick=()=>{bookingDrawer&&(bookingDrawer.style.display="none");const w=document.getElementById("breakdown-grand-total")?document.getElementById("breakdown-grand-total").innerText:e.priceStr;xe=e,pe=`BCN-2026-${Math.floor(1e4+Math.random()*9e4)}`;const D=e.style==="couple"||e.category==="beaches";let R="wanderworld@upi",ee="WanderWorld Travels";D?(R=localStorage.getItem("beacon_planner_upi")||"rahul@upi",ee=localStorage.getItem("beacon_planner_business")||"Rahul Mehta"):(R="wanderlust@upi",ee="Wanderlust Travels");const fe=document.getElementById("checkout-payable-amount"),Oe=document.getElementById("checkout-package-title"),Re=document.getElementById("checkout-booking-id"),At=document.getElementById("checkout-upi-display"),dn=document.getElementById("checkout-note-display"),pn=document.getElementById("checkout-qr-code");if(fe&&(fe.innerText=w),Oe&&(Oe.innerText=e.title),Re&&(Re.innerText=pe),At&&(At.innerText=R),dn&&(dn.innerText=pe),pn){const Zn=`upi://pay?pa=${R}&pn=${encodeURIComponent(ee)}&am=${w.replace(/[^\d]/g,"")}&tn=${pe}`;pn.src=`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(Zn)}`}const gn=document.getElementById("checkout-payment-modal");gn&&(gn.style.display="flex")})};let p=null;const Un=()=>{if(!p)return;let e=0,t=0;p.travellers.infants,p.passengerProfiles.forEach(a=>{if(a.type==="Infant")return;const n=parseInt(a.age);!isNaN(n)&&n<=12?(a.type="Child",t++):(a.type="Adult",e++)}),p.travellers.adults=e,p.travellers.children=t},_t=()=>{if(!p)return{basePrice:0,upgradesPrice:0,mealsPrice:0,addonsPrice:0,discount:0,subtotal:0,taxes:0,grandTotal:0};Un();const e=p.pkg,t=p.travellers.adults||0,a=p.travellers.children||0;p.travellers.infants;const n=e.priceNum||22e3,o=Math.round(n*.68),s=n*t,i=o*a,c=s+i+0;let g=0;p.stayUpgrades.includes("sea-view")&&(g+=3e3),p.stayUpgrades.includes("extra-bed")&&(g+=1200*(e.duration||4)),p.transportUpgrades.includes("suv")&&(g+=2e3),p.transportUpgrades.includes("pickup-vip")&&(g+=1500);let u=0;if(p.mealMode==="same")p.passengerProfiles.forEach(b=>{if(b.type==="Infant")return;p.mealPreferences[b.id]==="Non-Veg"&&(u+=650)});else{const b=e.duration||4;for(let I=1;I<=b;I++){const E=p.dayWiseMeals[I]||{},C=E.lunch||{};p.passengerProfiles.forEach(z=>{C[z.id]==="Non-Vegetarian"&&(u+=150)});const T=E.dinner||{};p.passengerProfiles.forEach(z=>{T[z.id]==="Non-Vegetarian"&&(u+=250)})}}let r=0;p.addOns.includes("scuba")&&(r+=2500*(t+a)),p.addOns.includes("dinner")&&(r+=3e3),p.addOns.includes("photos")&&(r+=4e3);const h=2e3,m=Math.max(0,c+g+u+r-h),v=Math.round(m*.05),y=m+v;return{basePrice:c,adultTotal:s,baseAdultPrice:n,childTotal:i,baseChildPrice:o,upgradesPrice:g,mealsPrice:u,addonsPrice:r,discount:h,subtotal:m,taxes:v,grandTotal:y}},te=()=>{if(!p)return;const e=JSON.parse(localStorage.getItem("beacon_bookings"))||[],t=e.findIndex(i=>i.id===p.bookingId),a={1:20,2:45,3:65,4:85,5:95}[p.currentStep]||10;let n="Traveller profiles incomplete";p.currentStep===2?n="Stay & transport selections pending":p.currentStep===3?n="Meals setup incomplete":p.currentStep===4?n="Trip add-ons config incomplete":p.currentStep===5&&(n="Review and confirm booking");const o=_t(),s={id:p.bookingId,packageTitle:p.pkg.title,imgUrl:p.pkg.imgUrl,dateRange:p.travelDate||"12 Oct – 15 Oct 2026",status:"draft",progress:a,progressDesc:n,travellersCount:p.passengerProfiles.length,estimatedTotal:o.grandTotal,state:p};t!==-1?e[t]=s:e.push(s),localStorage.setItem("beacon_bookings",JSON.stringify(e)),Ze()},ka=e=>{p={bookingId:`BCN-2026-${Math.floor(1e4+Math.random()*9e4)}`,pkg:e,status:"draft",currentStep:1,travelDate:"12 Oct – 15 Oct 2026",travellers:{adults:1,children:0,infants:0},passengerProfiles:[{id:1,type:"Adult",name:"Aditya Kasod",age:21,gender:"Male",collapsed:!0,isPrimary:!0}],stayUpgrades:[],transportUpgrades:[],mealMode:"same",mealPreferences:{1:"Vegetarian",2:"Non-Veg",3:"Vegetarian",4:"No Meal"},dayWiseMeals:{1:{lunch:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"},dinner:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"}},2:{breakfast:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"},dinner:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"}},3:{lunch:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"},dinner:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"}},4:{breakfast:{1:"Vegetarian",2:"Vegetarian",3:"Vegetarian"}}},addOns:[],timestamp:Date.now()},te(),_("mobile-booking"),Ee()},Hn=e=>{const a=(JSON.parse(localStorage.getItem("beacon_bookings"))||[]).find(n=>n.id===e);a&&a.state?(p=a.state,_("mobile-booking"),Ee()):showToast("⚠️ Could not restore draft booking state.")},Ee=()=>{const e=document.getElementById("view-mobile-booking");if(!e||!p)return;const t=p.pkg,a=p.currentStep,n=_t(),o=["Travellers","Stay","Meals","Add-ons","Review"];let s='<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); padding:12px 16px; border-radius:10px;">';o.forEach((l,f)=>{const x=f+1,P=a===x,ie=a>x;let ue=`<span style="width:20px; height:20px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px; font-weight:800; border:1px solid var(--text-slate); color:var(--text-slate); background:transparent;">${x}</span>`;P?ue=`<span style="width:22px; height:22px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px; font-weight:800; border:1px solid var(--accent-cyan); color:var(--bg-dark); background:var(--accent-cyan); box-shadow:0 0 6px var(--accent-cyan);">${x}</span>`:ie&&(ue='<span style="width:20px; height:20px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px; font-weight:800; border:1px solid var(--accent-cyan); color:var(--accent-cyan); background:transparent;">✓</span>'),s+=`
                <div style="display:flex; flex-direction:column; align-items:center; gap:4px; flex:1;">
                    ${ue}
                    <span style="font-size:8.5px; font-weight:800; color:${P?"var(--accent-cyan)":"var(--text-slate)"}; text-transform:uppercase;">${l}</span>
                </div>
                                        ...
                                        <div class="day-customize-body" style="display:none; padding:12px 14px; background:rgba(7,10,20,0.4); border-top:1px solid rgba(255,255,255,0.04);">
                                            ...
                                            <div style="margin-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.02); padding-bottom:10px;">
                                                ...
                                                `:`
                                                    ...
                                                    <div style="display:flex; flex-direction:column; gap:8px;">
                                                        ${p.passengerProfiles.map(x=>{if(x.type==="Infant")return"";const P=(f.lunch||{})[x.id]||"Vegetarian";return`
                                                                <div style="display:flex; justify-content:space-between; align-items:center;">
                                                                    ...
                                                                    <select class="sel-day-meal-choice" data-day="${l}" data-meal="lunch" data-pid="${x.id}" style="background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.08); padding:4px 8px; border-radius:4px; color:#fff; font-size:11px;">
                                                                        <option value="Vegetarian" ${P==="Vegetarian"?"selected":""}>Vegetarian (Included)</option>
                                                                        <option value="Non-Vegetarian" ${P==="Non-Vegetarian"?"selected":""}>Non-Vegetarian (+₹150)</option>
                                                                        <option value="Jain" ${P==="Jain"?"selected":""}>Jain (Included)</option>
                                                                    </select>
                                                                </div>
                                                            `}).join("")}
                                                    </div>
                                            ...
                                            <div>
                                                ...
                                                `:`
                                                    <div style="display:flex; flex-direction:column; gap:8px;">
                                                        ${p.passengerProfiles.map(x=>{if(x.type==="Infant")return"";const P=(f.dinner||{})[x.id]||"Vegetarian";return`
                                                                <div style="display:flex; justify-content:space-between; align-items:center;">
                                                                    <span style="font-size:11.5px; color:var(--text-slate);">${x.name}:</span>
                                                                    <select class="sel-day-meal-choice" data-day="${l}" data-meal="dinner" data-pid="${x.id}" style="background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.08); padding:4px 8px; border-radius:4px; color:#fff; font-size:11px;">
                                                                        <option value="Vegetarian" ${P==="Vegetarian"?"selected":""}>Vegetarian (Included)</option>
                                                                        <option value="Non-Vegetarian" ${P==="Non-Vegetarian"?"selected":""}>Non-Vegetarian (+₹250)</option>
                                                                        <option value="Jain" ${P==="Jain"?"selected":""}>Jain (Included)</option>
                                                                    </select>
                                                                </div>
                                                            `}).join("")}
        ...
        `;const d=document.getElementById("btn-m-wizard-back");d&&(d.onclick=()=>{a>1?(p.currentStep-=1,te(),Ee()):window.innerWidth<768?(_("package-details",!0),ht(t)):Se(t)});const c=document.getElementById("btn-m-wizard-continue");c&&(c.onclick=()=>{a<5?(p.currentStep+=1,te(),Ee()):On()}),document.querySelectorAll(".btn-jump-step").forEach(l=>{l.onclick=()=>{const f=parseInt(l.dataset.step);p.currentStep=f,te(),Ee()}});const u=document.getElementById("m-sticky-total-expand-btn"),r=document.getElementById("price-breakdown-popup-sheet"),h=document.getElementById("btn-close-breakdown-popup");u&&r&&(u.onclick=()=>{r.style.display="block"}),h&&r&&(h.onclick=()=>{r.style.display="none"});const m=document.getElementById("m-wizard-date-select");m&&(m.onchange=()=>{p.travelDate=m.value,te()});const v=document.getElementById("btn-infant-dec"),y=document.getElementById("btn-infant-inc");v&&y&&(v.onclick=()=>{if(p.travellers.infants>0){p.travellers.infants-=1;const l=p.passengerProfiles.map(f=>f.type).lastIndexOf("Infant");l!==-1&&p.passengerProfiles.splice(l,1),te(),Ee()}},y.onclick=()=>{if(p.travellers.infants<5){p.travellers.infants+=1;const l=Math.max(...p.passengerProfiles.map(f=>f.id),0)+1;p.passengerProfiles.push({id:l,type:"Infant",name:"",age:1,gender:"Female",collapsed:!1}),te(),Ee()}});const b=document.getElementById("btn-add-trip-mate");b&&(b.onclick=()=>{const l=Math.max(...p.passengerProfiles.map(f=>f.id),0)+1;p.passengerProfiles.forEach(f=>f.collapsed=!0),p.passengerProfiles.push({id:l,type:"Adult",name:"",age:"",gender:"Male",collapsed:!1}),te(),Ee()}),document.querySelectorAll(".btn-delete-p-card").forEach(l=>{l.onclick=()=>{const f=parseInt(l.dataset.id);p.passengerProfiles=p.passengerProfiles.filter(x=>x.id!==f),te(),Ee()}}),document.querySelectorAll(".btn-edit-p-card").forEach(l=>{l.onclick=()=>{const f=parseInt(l.dataset.id);p.passengerProfiles.forEach(x=>{x.collapsed=x.id!==f}),Ee()}}),document.querySelectorAll(".btn-collapse-p-card").forEach(l=>{l.onclick=()=>{const f=parseInt(l.dataset.id),x=p.passengerProfiles.find(P=>P.id===f);x&&(x.collapsed=!0,F(f),te(),Ee())}}),document.querySelectorAll(".input-p-name").forEach(l=>{l.onchange=()=>{const f=parseInt(l.dataset.id),x=p.passengerProfiles.find(P=>P.id===f);x&&(x.name=l.value),te()}}),document.querySelectorAll(".input-p-age").forEach(l=>{l.onchange=()=>{const f=parseInt(l.dataset.id),x=p.passengerProfiles.find(P=>P.id===f);x&&(x.age=parseInt(l.value)||0),te()}}),document.querySelectorAll(".select-p-gender").forEach(l=>{l.onchange=()=>{const f=parseInt(l.dataset.id),x=p.passengerProfiles.find(P=>P.id===f);x&&(x.gender=l.value),te()}}),document.querySelectorAll(".select-p-extrabed").forEach(l=>{l.onchange=()=>{const f=parseInt(l.dataset.id),x=p.passengerProfiles.find(P=>P.id===f);x&&(x.extraBed=l.value),x&&l.value==="Yes"&&(p.stayUpgrades.includes("extra-bed")||p.stayUpgrades.push("extra-bed")),te()}});function F(l){const f=document.querySelector(`.input-p-name[data-id="${l}"]`);if(f){const x=p.passengerProfiles.find(P=>P.id===l);if(x){const P=f.value,ie=parseInt(document.querySelector(`.input-p-age[data-id="${l}"]`).value)||0,ue=document.querySelector(`.select-p-gender[data-id="${l}"]`).value;x.name=P,x.age=ie,x.gender=ue}}}document.querySelectorAll(".cb-upgrade-stay").forEach(l=>{l.onchange=()=>{const f=l.value;l.checked?p.stayUpgrades.includes(f)||p.stayUpgrades.push(f):p.stayUpgrades=p.stayUpgrades.filter(x=>x!==f),ge(),te()}}),document.querySelectorAll(".cb-upgrade-trans").forEach(l=>{l.onchange=()=>{const f=l.value;l.checked?p.transportUpgrades.includes(f)||p.transportUpgrades.push(f):p.transportUpgrades=p.transportUpgrades.filter(x=>x!==f),ge(),te()}}),document.querySelectorAll(".meal-mode-card").forEach(l=>{l.onclick=()=>{const f=l.dataset.mode;p.mealMode=f,te(),Ee()}}),document.querySelectorAll(".btn-meal-chip").forEach(l=>{l.onclick=()=>{const f=parseInt(l.dataset.pid),x=l.dataset.val;p.mealPreferences[f]=x,ge(),te(),Ee()}});const oe=document.querySelectorAll(".day-customize-accordion");oe.forEach(l=>{const f=l.querySelector(".day-customize-header"),x=l.querySelector(".day-customize-body");f&&x&&(f.onclick=()=>{const P=x.style.display==="block";oe.forEach(ie=>{ie.querySelector(".day-customize-body").style.display="none",ie.classList.remove("active-day-acc")}),P||(x.style.display="block",l.classList.add("active-day-acc"))})}),document.querySelectorAll(".sel-day-meal-choice").forEach(l=>{l.onchange=()=>{const f=parseInt(l.dataset.day),x=l.dataset.type||l.dataset.meal,P=parseInt(l.dataset.pid);p.dayWiseMeals[f]||(p.dayWiseMeals[f]={lunch:{},dinner:{}}),p.dayWiseMeals[f][x]||(p.dayWiseMeals[f][x]={}),p.dayWiseMeals[f][x][P]=l.value,ge(),te()}}),document.querySelectorAll(".cb-addon-trip").forEach(l=>{l.onchange=()=>{const f=l.value;l.checked?p.addOns.includes(f)||p.addOns.push(f):p.addOns=p.addOns.filter(x=>x!==f),ge(),te()}});function ge(){const l=document.getElementById("lbl-sticky-wizard-total");if(l){const f=_t();l.style.color="#ef4444",l.classList.add("pop-bounce"),setTimeout(()=>{l.innerText="₹"+f.grandTotal.toLocaleString(),l.style.color="var(--accent-cyan)",l.classList.remove("pop-bounce")},200)}}},On=()=>{if(!p)return;const e=_t(),t=JSON.parse(localStorage.getItem("beacon_bookings"))||[],a=t.findIndex(m=>m.id===p.bookingId);a!==-1&&(t[a].status="🟡 PAYMENT VERIFICATION PENDING",t[a].progress=100,t[a].progressDesc="Payment details submitted for verification",t[a].state.status="🟡 PAYMENT VERIFICATION PENDING"),localStorage.setItem("beacon_bookings",JSON.stringify(t)),Ze();const n="wanderworld@upi",o="WanderWorld Travels",s="₹"+e.grandTotal.toLocaleString(),i=document.getElementById("checkout-payable-amount"),d=document.getElementById("checkout-package-title"),c=document.getElementById("checkout-booking-id"),g=document.getElementById("checkout-upi-display"),u=document.getElementById("checkout-note-display"),r=document.getElementById("checkout-qr-code");if(i&&(i.innerText=s),d&&(d.innerText=p.pkg.title),c&&(c.innerText=p.bookingId),g&&(g.innerText=n),u&&(u.innerText=p.bookingId),r){const m=`upi://pay?pa=${n}&pn=${encodeURIComponent(o)}&am=${e.grandTotal}&tn=${p.bookingId}`;r.src=`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(m)}`}const h=document.getElementById("checkout-payment-modal");h&&(h.style.display="flex"),_("bookings",!0)},Gn=e=>{const t=JSON.parse(localStorage.getItem("beacon_bookings"))||[],a=t.find(n=>n.id===e);a&&(a.status="Confirmed",Y(a),localStorage.setItem("beacon_bookings",JSON.stringify(t)),showToast("🎉 Payment verified! Booking Confirmed & Receipt generated."),Ze())},ln=e=>{const a=(JSON.parse(localStorage.getItem("beacon_bookings"))||[]).find(u=>u.id===e);if(!a||!a.receiptSnapshot){showToast("⚠️ Receipt snapshot details not available.");return}const n=a.receiptSnapshot,o=n.planner,s=n.pricing,i=document.getElementById("view-receipt-viewer");if(!i)return;i.innerHTML=`
        ...
        `,e.appendChild(t),e.scrollTop=e.scrollHeight,Wn(),showToast("📩 Aarav Mehta (Planner) sent a new message in Enquiries."),localStorage.setItem("beacon_enquiry_simulated","true")},Yn=()=>{const e=localStorage.getItem("beacon_unread_enquiry")==="true",t=localStorage.getItem("beacon_enquiry_simulated")==="true";if(e){if(xt&&(xt.style.display="block"),t){const a=document.querySelector("#enquiry-ws-1 .message-thread");if(a&&!a.querySelector(".unread-msg")){const n=document.createElement("div");n.className="msg-bubble planner unread-msg",n.style.borderLeft="3px solid var(--accent-cyan)",n.innerHTML=`
            ...
            `,n.appendChild(i),t.value="",n.scrollTop=n.scrollHeight,setTimeout(()=>{const d=document.createElement("div");d.className="msg-bubble planner",d.innerHTML=`
                ...
                `;const Ie=W.querySelector(".faq-header"),Le=W.querySelector(".faq-body"),w=W.querySelector(".faq-arrow");Ie.addEventListener("click",()=>{const k=Le.style.display==="block";Le.style.display=k?"none":"block",w.style.transform=k?"rotate(0deg)":"rotate(180deg)"}),b.appendChild(W)})};y&&(y.innerHTML="",v.forEach(B=>{const L=document.createElement("span");L.innerText=B,L.style.padding="5px 12px",L.style.borderRadius="20px",L.style.fontSize="11px",L.style.fontWeight="700",L.style.cursor="pointer",L.style.border="1px solid rgba(255,255,255,0.08)",(()=>{I===B?(L.style.background="var(--accent-cyan)",L.style.color="var(--bg-dark)",L.style.borderColor="var(--accent-cyan)"):(L.style.background="rgba(255,255,255,0.02)",L.style.color="#fff",L.style.borderColor="rgba(255,255,255,0.08)")})(),L.addEventListener("click",()=>{I=B,document.querySelectorAll("#kb-categories-wrap span").forEach(O=>{O.style.background="rgba(255,255,255,0.02)",O.style.color="#fff",O.style.borderColor="rgba(255,255,255,0.08)"}),L.style.background="var(--accent-cyan)",L.style.color="var(--bg-dark)",L.style.borderColor="var(--accent-cyan)",E()}),y.appendChild(L)}));const C=document.getElementById("kb-search-input");C&&C.addEventListener("input",E),E();const T=["Booking Issue","Payment Issue","Refund Request","Package Change","Cancellation","Planner Complaint","Safety Concern","General Question","Other"];g&&(g.innerHTML="",T.forEach(B=>{const L=document.createElement("option");L.value=B,L.innerText=B,g.appendChild(L)}));let z="Low",V=1;const le=()=>{c&&(V=1,q(),c.style.display="flex")};s&&s.addEventListener("click",le),i&&i.addEventListener("click",le),d&&c&&d.addEventListener("click",()=>{c.style.display="none"});const F=document.querySelectorAll(".priority-select-btn");F.forEach(B=>{B.addEventListener("click",()=>{F.forEach($=>{$.style.background="rgba(255,255,255,0.02)",$.style.borderColor="rgba(255,255,255,0.08)",$.style.color="#fff"}),B.style.background="rgba(0, 203, 224, 0.05)",B.style.borderColor="var(--accent-cyan)",B.style.color="var(--accent-cyan)",z=B.dataset.priority;const L=document.getElementById("critical-priority-warning");L&&(L.style.display=z==="Critical"?"block":"none")})});const q=()=>{var L,$;const B=document.getElementById("wizard-step-label");if(B&&(B.innerText=`Step ${V} of 4: Setup details`),document.querySelectorAll(".wizard-pane").forEach((O,ce)=>{O.style.display=ce+1===V?"block":"none"}),V===4){const O=((L=document.getElementById("wizard-subject"))==null?void 0:L.value)||"",ce=(($=document.getElementById("wizard-category"))==null?void 0:$.value)||"",W=document.getElementById("review-subject"),Ie=document.getElementById("review-cat"),Le=document.getElementById("review-priority");W&&(W.innerText=O),Ie&&(Ie.innerText=ce),Le&&(Le.innerText=z)}};(ge=document.getElementById("btn-wizard-next-1"))==null||ge.addEventListener("click",()=>{V=2,q()}),(l=document.getElementById("btn-wizard-back-2"))==null||l.addEventListener("click",()=>{V=1,q()}),(f=document.getElementById("btn-wizard-next-2"))==null||f.addEventListener("click",()=>{V=3,q()}),(x=document.getElementById("btn-wizard-back-3"))==null||x.addEventListener("click",()=>{V=2,q()}),(P=document.getElementById("btn-wizard-next-3"))==null||P.addEventListener("click",()=>{var $,O;const B=(($=document.getElementById("wizard-subject"))==null?void 0:$.value)||"",L=((O=document.getElementById("wizard-desc"))==null?void 0:O.value)||"";if(!B.trim()||!L.trim()){alert("Please enter Subject and Description details.");return}V=4,q()}),(ie=document.getElementById("btn-wizard-back-4"))==null||ie.addEventListener("click",()=>{V=3,q()});let G=[{id:"BCN-SUP-2026-000201",subject:"Double charged on Kashmir Honeymoon booking",category:"Payment Issue",priority:"High",status:"In Progress",createdDate:"2026-07-31 10:20 AM",description:"I initiated two transactions due to first one returning a network gateway error. Both payments are success in my bank diary.",timeline:[{status:"Ticket Created",date:"2026-07-31 10:20 AM",by:"You"},{status:"Assigned",date:"2026-07-31 10:30 AM",by:"System"},{status:"In Progress",date:"2026-07-31 11:00 AM",by:"Siddharth Roy"}],conversations:[{sender:"Traveler",message:"Hello, I paid twice. Please verify refund options.",time:"10:20 AM"},{sender:"Support",message:"Hi Aditya, checking logs. Yes, we see two duplicate UPI credits. Initiating refund for the duplicate transaction.",time:"11:00 AM"}]}],U=null;(ue=document.getElementById("btn-wizard-submit"))==null||ue.addEventListener("click",()=>{const B=document.getElementById("wizard-subject").value,L=document.getElementById("wizard-desc").value,$=document.getElementById("wizard-category").value;document.getElementById("wizard-bkg-id").value;const O={id:`BCN-SUP-2026-000${202+G.length}`,subject:B,category:$,priority:z,status:"Open",createdDate:"Just now",description:L,timeline:[{status:"Ticket Created",date:"Just now",by:"You"}],conversations:[{sender:"Traveler",message:L,time:"Just now"}]};G.push(O),c&&(c.style.display="none"),document.getElementById("wizard-subject").value="",document.getElementById("wizard-desc").value="",document.getElementById("wizard-bkg-id").value="",showToast(`🎫 Ticket ${O.id} created successfully!`),u&&u.click()});const Z=()=>{const B=document.getElementById("customer-tickets-container");B&&(B.innerHTML="",G.forEach(L=>{const $=document.createElement("div");$.style.background=U===L.id?"rgba(0, 203, 224, 0.05)":"rgba(255,255,255,0.01)",$.style.border=U===L.id?"2.5px solid var(--accent-cyan)":"1px solid rgba(255,255,255,0.08)",$.style.borderRadius="8px",$.style.padding="12px",$.style.cursor="pointer",$.style.textAlign="left",$.innerHTML=`
