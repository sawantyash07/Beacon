(function(){const Y=document.createElement("link").relList;if(Y&&Y.supports&&Y.supports("modulepreload"))return;for(const z of document.querySelectorAll('link[rel="modulepreload"]'))S(z);new MutationObserver(z=>{for(const M of z)if(M.type==="childList")for(const _ of M.addedNodes)_.tagName==="LINK"&&_.rel==="modulepreload"&&S(_)}).observe(document,{childList:!0,subtree:!0});function ee(z){const M={};return z.integrity&&(M.integrity=z.integrity),z.referrerPolicy&&(M.referrerPolicy=z.referrerPolicy),z.crossOrigin==="use-credentials"?M.credentials="include":z.crossOrigin==="anonymous"?M.credentials="omit":M.credentials="same-origin",M}function S(z){if(z.ep)return;z.ep=!0;const M=ee(z);fetch(z.href,M)}})();window.addEventListener("error",D=>{console.error("Global JS Error Captured in app.js:",D.message,"at",D.filename,":",D.lineno)});window.showToast=D=>{const Y=document.getElementById("playground-toast");Y?(Y.innerText=D,Y.classList.add("active"),setTimeout(()=>{Y.classList.remove("active")},2200)):console.log("Toast:",D)};let q=[];const vt=D=>D.title.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,""),eo=D=>{const Y=vt(D),ee=`${window.location.origin}/?package=${Y}`,S=D.priceStr||D.price,z=D.basePrice||Math.round(D.priceNum*1.11),M=Math.round((z-D.priceNum)/z*100),_="₹"+z.toLocaleString(),Te=(D.highlights?D.highlights.split(","):["Luxury Stays","Guided Activities","Transfers Included"]).slice(0,4).map(K=>K.trim()).join(" • ");let $e="";return M>0&&($e=`
...
`)}),e.innerHTML=n;const s=`
        ...
        `,a.querySelector("#btn-wait-close").onclick=()=>{a.style.display="none",clearInterval(gt)},gt=setInterval(()=>{const s=(JSON.parse(localStorage.getItem("beacon_payment_attempts"))||[]).find(i=>i.bookingId===e);s&&s.status==="PLANNER_CONFIRMED"?(clearInterval(gt),n.innerHTML=`
                ...
                `,a.querySelector("#btn-wait-view-booking").onclick=()=>{a.style.display="none",Ze(),W("bookings",!0)}):s&&s.status==="PAYMENT_REVIEW_REQUIRED"&&(clearInterval(gt),n.innerHTML=`
                    <div style="font-size: 48px; margin-bottom: 20px;">❌</div>
        ...
        `},Vn=e=>{let t=JSON.parse(localStorage.getItem("beacon_recent_views"))||[];t=t.filter(a=>a.title!==e.title),t.unshift({title:e.title,viewedAt:Date.now()}),t=t.slice(0,15),localStorage.setItem("beacon_recent_views",JSON.stringify(t))},jn=e=>{const t=document.getElementById("similar-packages-grid");if(!t)return;let a=q.filter(i=>i.title!==e.title);a.forEach(i=>{let d=0;i.category===e.category&&(d+=40),i.style===e.style&&(d+=25);const c=Math.abs(i.priceNum-e.priceNum)/e.priceNum;c<=.3&&(d+=Math.round((1-c)*25)),Math.abs(i.duration-e.duration)<=1&&(d+=10),i.similarityScore=d}),a.sort((i,d)=>d.similarityScore-i.similarityScore);const n=a.slice(0,4);let o="";n.forEach(i=>{const d=Math.min(96,Math.max(78,Math.round(75+i.similarityScore/1.5))),c=X.includes(i.title),g=F.some(r=>r.title===i.title);let u="Popular choice";i.category===e.category?u="Same Destination":i.style===e.style?u=`Similar ${i.style}`:Math.abs(i.priceNum-e.priceNum)/e.priceNum<=.15&&(u="Similar budget"),o+=`
            ...
            `}),t.innerHTML=o,t.querySelectorAll(".rec-travel-card").forEach((i,d)=>{const c=n[d];i.addEventListener("click",r=>{r.target.closest("button")||Se(c)});const g=i.querySelector(".btn-rec-save");g.addEventListener("click",r=>{r.stopPropagation(),We(c.title);const h=X.includes(c.title);g.innerHTML=h?"❤️":"♡",g.style.color=h?"var(--accent-cyan)":"var(--text-slate)";const m=document.querySelector(".details-save-btn");m&&c.title===e.title&&(m.innerHTML=h?"❤️ Saved":"♡ Save")});const u=i.querySelector(".btn-rec-compare");u.addEventListener("click",r=>{r.stopPropagation();const h=F.some(v=>v.title===c.title);dt(c,!h);const m=F.some(v=>v.title===c.title);u.innerHTML=m?"✓":"+",u.style.color=m?"var(--accent-cyan)":"var(--text-slate)"})})},nn=(e,t)=>{const a=document.getElementById("view-planner-profile");if(!a)return;W("planner-profile",!0);const n=e.name==="Rahul Mehta",o=q.filter(c=>c.title===t.title?!1:n?c.category==="beaches"||c.style==="couple":c.category==="mountains"||c.category==="nature"||c.category==="spiritual").slice(0,4);let s="";o.forEach(c=>{s+=`
