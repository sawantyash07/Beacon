...
export const testimonials = [
  { id: 1, name: 'Sarah Chen', role: 'Travel Agency Owner', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&q=80', text: 'Beacon Planner transformed how we manage our travel business. The dashboard is intuitive and our clients love the experience.', rating: 5 },
...
export const faqs = [
  { question: 'What is Beacon Planner?', answer: 'Beacon Planner is an AI-powered travel management platform that helps travel planners create packages, manage bookings, communicate with travelers, and grow their business.' },
  { question: 'Do I need technical skills to use Beacon Planner?', answer: 'Not at all. Beacon Planner is designed for travel professionals with an intuitive interface. You can create and publish packages in minutes.' },
  { question: 'Can I connect my social media accounts?', answer: 'Yes! Beacon Planner supports Instagram, Facebook, YouTube, and WhatsApp Business. New packages can be automatically shared to connected accounts.' },
...
const LOCAL_STORAGE_KEY = 'beacon_planner_packages';
...
const savedPackages = typeof window !== 'undefined' ? localStorage.getItem(LOCAL_STORAGE_KEY) : null;
let parsedPackages = savedPackages ? JSON.parse(savedPackages) : defaultPackages;

export let packages = parsedPackages.map((pkg: any) => {
  if (!pkg.plannerId) {
    pkg.plannerId = pkg.id === 'PKG-003' ? 'other-planner-id' : 'mock-planner-id-123';
  }
  return pkg;
});
...
const BOOKINGS_STORAGE_KEY = 'beacon_planner_bookings';
const savedBookings = typeof window !== 'undefined' ? localStorage.getItem(BOOKINGS_STORAGE_KEY) : null;
let parsedBookings = savedBookings ? JSON.parse(savedBookings) : defaultBookings;

export let bookings = parsedBookings.map((b: any) => {
  if (!b.plannerId) {
    b.plannerId = b.packageId === 'PKG-003' ? 'other-planner-id' : 'mock-planner-id-123';
  }
  return b;
});
...
export function saveTripGroupsToStorage() {
  if (typeof window !== 'undefined') {
    localStorage.setItem('beacon_planner_trip_groups', JSON.stringify(tripGroups));
  }
}
...
export function addMockBooking(newBooking: any) {
  bookings.unshift(newBooking)
  saveBookingsToStorage()
  addBookingToTripGroup(newBooking)
  return newBooking;
}

const TRIP_GROUPS_STORAGE_KEY = 'beacon_planner_trip_groups';
const defaultTripGroups = [
  {
    id: 'GRP-001',
...
const savedTripGroups = typeof window !== 'undefined' ? localStorage.getItem(TRIP_GROUPS_STORAGE_KEY) : null;
export let tripGroups = savedTripGroups ? JSON.parse(savedTripGroups) : defaultTripGroups;

if (typeof window !== 'undefined' && !savedTripGroups) {
  localStorage.setItem(TRIP_GROUPS_STORAGE_KEY, JSON.stringify(tripGroups));
}
...
export const transactions = [
  { id: 'TXN-9012', description: 'Booking BKG-7821', amount: 155000, type: 'credit', date: '2026-07-20' },
  { id: 'TXN-9013', description: 'Withdrawal to bank', amount: -250000, type: 'debit', date: '2026-07-21' },
  { id: 'TXN-9014', description: 'Booking BKG-7823', amount: 85000, type: 'credit', date: '2026-07-23' },
  { id: 'TXN-9015', description: 'Booking BKG-7825', amount: 95000, type: 'credit', date: '2026-07-25' },
  { id: 'TXN-9016', description: 'Refund BKG-7824', amount: -115000, type: 'debit', date: '2026-07-24' },
]

export const socialAccounts = [
  { platform: 'Instagram', connected: true, handle: '@beacon_planner', icon: 'Instagram' },
  { platform: 'Facebook', connected: true, handle: 'Planner Travel', icon: 'Facebook' },
...
export const activityFeed = [
  ...
  { id: 3, type: 'payment', text: 'Payment received: ₹1,55,000', time: '2026-07-25T14:20:00' },
...
export const navItems = [
  ...
  { type: 'divider' },
  { label: 'Enquiries', path: '/dashboard/inquiries', icon: 'Mail' },
  { label: 'Packages', path: '/dashboard/packages', icon: 'Package' },
  { label: 'Bookings', path: '/dashboard/bookings', icon: 'Calendar' },
  { label: 'Payments', path: '/dashboard/payments', icon: 'CreditCard' },
  { label: 'All Trips', path: '/dashboard/trip-operations', icon: 'Bus' },
  { label: 'Travellers', path: '/dashboard/travellers', icon: 'Users' },
  { type: 'divider' },
  { label: 'Analytics', path: '/dashboard/analytics', icon: 'BarChart3' },
  { label: 'Reviews', path: '/dashboard/reviews', icon: 'Star' },
  { label: 'Marketing Hub', path: '/dashboard/marketing', icon: 'Share2' },
  { type: 'divider' },
  { label: 'Business Profile', path: '/dashboard/business-profile', icon: 'Building2' },
  { label: 'Team Management', path: '/dashboard/team', icon: 'Users' },
  { label: 'Support Center', path: '/dashboard/support', icon: 'HelpCircle' },
