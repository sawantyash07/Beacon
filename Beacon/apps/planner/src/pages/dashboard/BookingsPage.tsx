...
export default function BookingsPage() {
  ...
  useEffect(() => {
    const loadBookings = async () => {
      try {
        const data = await fetchBookings({ plannerId: user?.id });
        const formatted = data.map((b: any) => ({
          id: b.id,
          traveler: b.travelerId === user?.id ? (user?.name || 'Primary Traveler') : 'Arjun Mehta', // standard guest traveler
          email: 'arjun.mehta@gmail.com',
          phone: '+91 98765 43210',
          package: b.package?.title || 'Unknown Package',
          destination: b.package?.destination || '',
          travelDate: b.travelDate ? b.travelDate.slice(0, 10) : 'TBD',
          bookingDate: b.createdAt ? b.createdAt.slice(0, 10) : 'TBD',
          travelersCount: b.passengerCount,
          amount: b.totalAmount,
          amountPaid: b.payment?.status === 'SUCCESS' ? b.totalAmount : 0,
  ...
  // Action Handlers
  const handleConfirmBookingExecution = async () => {
    if (!confirmTargetBooking) return
    try {
      await updateBookingStatus(confirmTargetBooking.id, 'CONFIRMED');
      setBookingList((prev) =>
        prev.map((b) =>
          b.id === confirmTargetBooking.id
            ? { ...b, status: 'confirmed', paymentStatus: 'success', amountPaid: b.amount }
            : b
        )
      )
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await updateBookingStatus(editStatusBooking.id, newStatus.toUpperCase());
      setBookingList((prev) =>
        prev.map((b) =>
          b.id === editStatusBooking.id
            ? { ...b, status: newStatus.toLowerCase(), paymentStatus: newPaymentStatus.toLowerCase() }
            : b
        )
      )
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      await updateBookingStatus(cancelTargetBooking.id, 'CANCELLED');
      setBookingList((prev) =>
        prev.map((b) =>
          b.id === cancelTargetBooking.id
            ? { ...b, status: 'cancelled', paymentStatus: 'failed', remainingBalance: 0 }
            : b
        )
      )
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface p-4 border border-border rounded-[18px] shadow-sm">
            <div className="relative flex-1">
              ...
              <label htmlFor="package-bookings-search" className="sr-only">Search bookings for this package</label>
              <input
                id="package-bookings-search"
                name="packageBookingsSearch"
                type="text"
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <label htmlFor="package-status-filter" className="sr-only">Filter by Booking Status</label>
              <select
              ...
              <label htmlFor="package-payment-filter" className="sr-only">Filter by Payment Status</label>
              <select
                id="package-payment-filter"
              ...
              <label htmlFor="package-sort-by" className="sr-only">Sort Bookings By</label>
              <select
                id="package-sort-by"
                name="packageSortBy"
                value={sortBy}
      ...
      {viewMode === 'entries' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface p-4 border border-border rounded-[18px] shadow-sm">
            <div className="relative flex-1">
              <Search className="w-4 h-4 text-muted absolute left-3.5 top-1/2 -translate-y-1/2" />
              <label htmlFor="unified-search-bookings" className="sr-only">Search all bookings</label>
              <input
                id="unified-search-bookings"
                name="unifiedSearchBookings"
                type="text"
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label htmlFor="unified-status-filter" className="sr-only">Filter by Booking Status</label>
              <select
                id="unified-status-filter"
                name="unifiedStatusFilter"
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="px-3 py-2 rounded-[10px] border border-border bg-page text-xs font-medium text-navy focus:outline-none"
              >
              ...
              <label htmlFor="unified-payment-filter" className="sr-only">Filter by Payment Status</label>
              <select
                id="unified-payment-filter"
                name="unifiedPaymentFilter"
                value={paymentFilter}
                onChange={(e) => setPaymentFilter(e.target.value)}
                className="px-3 py-2 rounded-[10px] border border-border bg-page text-xs font-medium text-navy focus:outline-none"
              >
              ...
              <label htmlFor="unified-sort-by" className="sr-only">Sort Bookings By</label>
              <select
                id="unified-sort-by"
                name="unifiedSortBy"
                value={sortBy}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-4 pt-2">
                <div>
                  <label htmlFor="edit-booking-status" className="block text-xs font-bold text-navy mb-1.5">Booking Status</label>
                  <select
                    id="edit-booking-status"
                    name="editBookingStatus"
                    value={newStatus}
                ...
                <div>
                  <label htmlFor="edit-payment-status" className="block text-xs font-bold text-navy mb-1.5">Payment Status</label>
                  <select
