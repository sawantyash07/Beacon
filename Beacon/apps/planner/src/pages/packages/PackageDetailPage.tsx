...
import { MapPin, Clock, Star, Check, ArrowLeft, Heart, CheckCircle2 } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div>
            <div className="glass rounded-[24px] p-8 border border-border/50 shadow-xl sticky top-32">
              <div className="mb-6 pb-6 border-b border-border">
                ...
                <div className="flex items-end gap-3">
                  <span className="text-4xl font-bold text-teal">
                    {formatCurrency(pkg.discountedPrice || pkg.basePrice || pkg.price || 0)}
                  </span>
                  {(pkg.discountedPrice && pkg.basePrice && pkg.basePrice > pkg.discountedPrice) && (
              ...
              {/* Dynamic Planner Profile Block */}
              <div className="bg-navy/5 border border-border/40 rounded-2xl p-4 mb-6 text-xs flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-teal/10 flex items-center justify-center text-teal font-extrabold shrink-0 shadow-sm uppercase">
                  {(pkg.planner?.profile?.companyName || pkg.planner?.name || 'CP').slice(0, 2)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    <h4 className="font-bold text-navy truncate">
                      {pkg.planner?.profile?.companyName || pkg.planner?.name || 'Concierge Partner'}
                    </h4>
                    {(pkg.planner?.profile?.isVerified ?? true) && (
                      <span className="text-teal shrink-0" title="Verified Travel Planner">
                        <CheckCircle2 className="w-3.5 h-3.5 fill-teal/10" />
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5 text-muted mt-0.5 font-semibold">
                    <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                    <span>{pkg.planner?.profile?.averageRating || 4.9} rating</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex justify-between text-muted">
                  ...
                  <span className="font-semibold text-navy">{pkg.groupSize || pkg.maxTravelers || 10}</span>
                </div>
                <div className="flex justify-between text-muted">
                  <span>Availability</span>
                  <span className="font-semibold text-green-600">
                    {pkg.status === 'PUBLISHED' ? 'Available' : pkg.status}
                  </span>
                </div>
              </div>
