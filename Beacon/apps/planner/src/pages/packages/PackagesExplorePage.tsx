              ...
              <div key={pkg.id} className="group relative h-full">
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    {(() => {
                      const totalBooked = pkg.bookings?.reduce((sum: number, b: any) => b.status === 'CONFIRMED' ? sum + b.passengerCount : sum, 0) || 0;
                      const remainingSeats = (pkg.groupSize || 12) - totalBooked;
                      ...
                      return (
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-1.5 text-xs font-semibold text-muted">
                            ...
                            {isSoldOut ? (
                              <span className="text-red-500 bg-red-50 px-2 py-0.5 rounded-md border border-red-200">Sold Out</span>
                            ) : (
                              <span className="text-teal">{remainingSeats} seats left</span>
                            )}
                          </div>
                          <span className="text-[11px] text-muted font-medium bg-black/5 px-2 py-0.5 rounded-md truncate max-w-[120px]">
                            {pkg.planner?.profile?.companyName || pkg.planner?.name || 'Concierge'}
                          </span>
                        </div>
                      );
                    })()}
